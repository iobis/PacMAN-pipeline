...
sample_data=read.table(config$meta$sampling$sample_data_file, sep=";", header=TRUE)
...
```{r, echo=F}
cat("Visual representation of the results of the QC on the raw data can be found in an html report in: results/", config$PROJECT, "/samples/multiqc_", config$RUN,".html", sep="")
...
sample=scan(trim_report,sep="\n",what="char(0)",skip=10, nlines=1)
sample=unlist(strsplit(sample, "\\s+"))
sample=as.numeric(sample[c(4, 7, 12, 17, 20)])
trimResults[i,]=sample
...
```{r, echo=F}
cutResults=data.frame(TotalReadPairsProcessed=NA, Read1withPrimer=NA, Read2withPrimer=NA, PairsTooShort=NA, PairsWritten=NA, BasePairsProcessed=NA, BasePairsWritten=NA)

for (i in 1:length(sample_ids)) {
cut_report=paste0("../../results/", config$PROJECT, "/runs/", config$RUN,"/06-report/cutadapt/", sample_ids[i], "_log.txt")
sample=scan(cut_report,sep="\n",what="char(0)",skip=7, nlines=12)
sample=unlist(strsplit(sample, "\\s+"))
sample=sample[c(5, 11, 18, 25, 31, 36, 51)]
...
```{r, echo=F}
cutResults=data.frame(TotalReadPairsProcessed=NA, ReadwithPrimer=NA, ReadsWritten=NA, BasePairsProcessed=NA, BasePairsWritten=NA)

for (i in 1:length(sample_ids)) {
cut_report=paste0("../../results/", config$PROJECT, "/runs/", config$RUN,"/06-report/cutadapt/", sample_ids[i], "_unpaired1_log.txt")
sample=scan(cut_report,sep="\n",what="char(0)",skip=7, nlines=6)
sample=unlist(strsplit(sample, "\\s+"))
sample=sample[c(4,8,14,19,24)]
sample=as.numeric(gsub(',', '', sample))
if (length(sample)==5){
cutResults[i,]=sample
rownames(cutResults)[i]=sample_ids[i]
...
cutResults_Percentage=round(cutResults[,c(2:3)]/cutResults[,c(1)]*100, digits=2)
cutResults_Percentage$BasePairsWritten=round(cutResults[,5]/cutResults[,4]*100, digits=2)
kable(cutResults_Percentage)
```
<br>
...
```{r, echo=F}
cat("results/", config$PROJECT, "/runs/", config$RUN,"/05-dwca/Occurrence_table.csv", sep="")
cat("results/", config$PROJECT, "/runs/", config$RUN,"/05-dwca/DNA_extension_table.csv", sep="")
...
cat("Of which", sum(occ_table$scientificName!="Biota"), "were known, and therefore", sum(occ_table$scientificName=="Biota"), "or", sum(occ_table$scientificName=="Biota")/nrow(occ_table)*100, "% remained unknown. The sequences of these occurrences can nevertheless be added to the database, and it is possible that their taxonomy will be defined when reference databases develop.")
