...
while (length(line) > 0 && !grepl("Input Read Pairs", line)) {
  line <- readLines(file, 1)
}
...
if (config$meta$sequencing$lib_layout=="Paired") { 
print("checking analysis of paired samples")
for (i in 1:length(sample_ids)) {
   #print(sample_ids[i])
  ...
  print(kable(cutResults))

cutResults_Percentage <- round(cutResults[,c(2:5)] / cutResults[,c(1)] * 100, digits = 2)
cutResults_Percentage$BasePairsWritten <- round(cutResults[,9] / cutResults[,6] * 100, digits = 2)
...
Next, taxonomy was assigned to the inferred reads using rdp
...
The amount of alignments that were made to the provided database:

#```{R}

cat(readLines(paste0("../../results/", config$PROJECT, "/runs/", config$RUN, "/06-report/bowtie2/bowtie2_", config$DATABASE$name, "_log.txt")), sep = '\n')

#```

Note, all alignments are treated as unpaired as any merging of reads would have happened in the dada2 pipeline. 
...
If chosen by the user (Blast: include: TRUE in the config file) the unknown sequences that did not align with the defined reference database, as well as those that did not have a match based on the blca analysis were aligned against the full ncbi-nt database. 
...
```{r, echo=F, results='asis'}
occ_path <- paste0("../../results/", config$PROJECT, "/runs/", config$RUN,"/05-dwca/Occurence_table.tsv")
